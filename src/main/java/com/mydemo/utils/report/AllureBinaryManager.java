package com.mydemo.utils.report;

import com.mydemo.utils.OSUtils;
import com.mydemo.utils.TerminalUtils;
import com.mydemo.utils.logs.LogsManager;
...
public class AllureBinaryManager {
    ...
    private static class LazyHolder {
        ...
        // Get the latest Allure version from GitHub releases
    ...
    public static void downloadAndExtract() {
        try {
            ...
            // Give execute permissions to the allure binary if not on Windows
            if (!OSUtils.getCurrentOS().equals(OSUtils.OS.WINDOWS)) {
    ...
    public static Path getExecutable() {
        ...
        // Determine the correct executable allure file based in allure folder on the operating system
    ...
    private static Path downloadZip(String version) {
        try {
            // URL for Allure ZIP file download
            ...
            // Folder to save the downloaded ZIP file
            //C:\Users\pc\.m2\repository\allure
            Path zipFile = Paths.get(AllureConstants.EXTRACTION_DIR.toString(), "allure-" + version + ".zip");
            // Download the Allure ZIP file if it doesn't exist and save it to the zipFile path
